...
class TestRecurrentTs(unittest.TestCase):
    def setUp(self):
        ...
        self.data_to_pad = pd.DataFrame(
            {
                "feature_1": [i for i in range(6)],
        ...
        self.data_to_pad_with_categorical = pd.DataFrame(
            {
                "one_hot_yes": [1, 1, 1, 1, 1, 1],
                "feature_2": [i for i in range(6)],
                "one_hot_no": [0, 0, 0, 0, 0, 0],
                "target": [i for i in range(6)],
            }
        )
        self.data_to_pad_with_multiple_categorical = pd.DataFrame(
            {
                "one_hot_yes": [1, 1, 1, 1, 1, 1],
                "feature_2": [i for i in range(6)],
                "one_hot_no": [0, 0, 0, 0, 0, 0],
    ...
    def test_len_padding(self):
        ...
        results = rec_instance._pad_ts(pandas_df=self.data_to_pad, desired_len=10)
        self.assertEqual(results.shape[0], 10)

    def test_zero_len_padding(self):
        rec_instance = TimeSeries(pandas_df=self.data_to_pad)
        results = rec_instance._pad_ts(
            pandas_df=self.data_to_pad, desired_len=6
        )  # len is the same as the original time series
    ...
    def test_padding_with_one_hot(self):
        rec_ts = TimeSeries(
            pandas_df=self.data_to_pad_with_categorical, one_hot_root_list=["one_hot"]
        )
        results = rec_ts._pad_ts(
            pandas_df=self.data_to_pad_with_categorical, desired_len=10
        )
    ...
    def test_padding_with_one_hot_multiple(self):
        rec_ts = TimeSeries(
            pandas_df=self.data_to_pad_with_categorical,
            one_hot_root_list=["one_hot", "other"],
        )

        results = rec_ts._pad_ts(
            pandas_df=self.data_to_pad_with_multiple_categorical, desired_len=10
        )
    ...
    def test_sample_ts(self):
        ...
        """
        rec_instance = TimeSeries(pandas_df=self.data_to_pad)
        results = rec_instance._sample_ts(pandas_df=self.data_to_pad, desired_len=3)
